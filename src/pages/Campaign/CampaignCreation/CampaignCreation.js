...
function CampaignCreation(props) {
  ...
  useEffect(() => {
    Api.fetchInternal("/auth/users").then((res) => setPlayers(res));

    Api.fetchInternal("/optionalrules").then((res) => setRules(res));
  }, []);
  ...
  const getTreeOption = (rule, index) => {
    ...
    return (
      <TreeItem
        ...
        label={
          <Tooltip disableFocusListener title={rule.description}>
  ...
  const handleSubmit = () => {
    ...
    Api.fetchInternal("/campaigns", {
      ...
      .then(() => {
        ...
        Api.fetchInternal("/campaigns").then((res) => {
            ...
            <FormControl className={classes.formControl} required>
              <Autocomplete
                ...
                renderTags={(tagValue, getTagProps) =>
                  tagValue.map((option, index) => <Chip label={option.username} {...getTagProps({ index })} />)
                }
          ...
          <Grid item sm={6}>
            <FormControl className={classes.formControl} required>
              <Autocomplete
                ...
                renderInput={(params) => <TextField {...params} label="Personajes" />}
              />
            </FormControl>
          </Grid>
          <Grid item lg={12}>
