...
function CampaignLogs(props) {
  ...
  useEffect(() => {
    ...
    Api.fetchInternal("/logs/" + props.campaignId).then((res) => {
      setLogs(res);
      const opts = [];
  ...
  return (
    <Box className={classes.container}>
      {logs.length > 0 && (
        <Paper variant="outlined" style={{ padding: "1rem", width: "100%" }}>
          <Box style={{ display: "flex", justifyContent: "space-between" }}>
            <Box style={{ display: "flex", alignItems: "center", paddingBottom: "1rem" }}>
              <Image
                src={logs[selectedCategory].discordData.guild.iconUrl}
                style={{ padding: ".5rem", maxWidth: "100px", maxHeight: "100px" }}
                errorStyle={{ borderRadius: "100%", height: "4rem", width: "4rem", margin: "0 1rem" }}
                ariaLabel={`Avatar de ${logs[selectedCategory].discordData.guild.name}`}
              />
              <Box>
                <Typography variant="h6">{logs[selectedCategory].discordData.guild.name}</Typography>
                <Typography variant="h6">
                  ...
                  >
                    {options.map((option) => (
                      <MenuItem style={{ textTransform: "capitalize" }} value={option.value}>
                        {option.label}
                      </MenuItem>
                    ))}
                  </Select>
                </Typography>
                <Typography>{"Total de mensajes: " + logs[selectedCategory].messages.length}</Typography>
              </Box>
            </Box>
          </Box>
          <Divider />
          <div className={classes.messageContainer} ref={(ref) => (chatBox = ref)} id="scrollableDiv">
            ...
            >
              {logs.length > 0 &&
                logs[selectedCategory].messages.slice(0, currentMessages).map((message) => {
                                      ...
                                      user: (node) => {
                                        if (dm && dm.metadata.discordId === node.id) {
                                          ...
                                          const selectedPlayer = players.filter(
                                            (player) => player.metadata.discordId === node.id,
                                          )[0];
                                          if (selectedPlayer) {
                                            const selectedCharacter = characters.filter(
                                              (character) => character.player === selectedPlayer.id,
                                            )[0];
                                            if (selectedCharacter) {
                                              return "@" + selectedCharacter.name;
                                            }
      ...
      <Box className={classes.game}></Box>
    </Box>
  );
}
