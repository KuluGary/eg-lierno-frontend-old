    ...
    useEffect(() => {
        if (characters && players && logs.length > 0) {
            ...
            Object.keys(userData).forEach((key) => {
                userData[key] = userData[key].sort(
            ...
            Object.keys(userData).forEach((key) => {
                ...
                const user = players.filter(
                    (player) => player.metadata.discordId === key,
                )[0];
                const userId = user.id;
                const char = characters.filter(
                    (character) => character.player === userId,
                )[0];
                ...
                entry.forEach((step) => {
                    if (
                        collection.data.findIndex(
                            (item) => item.x === step.date.split(" ")[0],
                        ) >= 0
                    ) {
