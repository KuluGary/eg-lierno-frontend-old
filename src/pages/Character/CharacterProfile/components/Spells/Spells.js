...
export default function Spells(props) {
  ...
  const SortButton = () => {
    switch (sortType) {
      case SORTBY.ASC:
        return (
          <Tooltip disableFocusListener title={"Mostrar preparados"}>
      ...
      default:
        return (
          <Tooltip disableFocusListener title={"Ordenar alfabéticamente"}>
            <IconButton onClick={toggleSort}>
            ...
            <Grid item lg={12} style={{ height: "calc(100% - 7rem)" }}>
              <Paper variant="outlined" style={{ height: "100%" }}>
                ...
                <Table>
                  <TableHead>
                    <TableRow>
                      {selectedCategory !== 0 && isPreparedClass() && <TableCell>¿Preparado?</TableCell>}
                      ...
                      tableItems
                        .sort((a, b) => {
                          if (sortType === SORTBY.ASC) {
                            ...
                            const aPrepared = spellIds?.find((spellData) => spellData.spellId === a._id).prepared;
                            const bPrepared = spellIds?.find((spellData) => spellData.spellId === b._id).prepared;
                            ...
                            {selectedCategory !== 0 && isPreparedClass() && (
                              <TableCell>
                                <Checkbox
                                  ...
                                  checked={
                                    spellIds?.filter((spellData) => spellData.spellId === spell._id)[0]?.prepared
                                  }
                                />
                              </TableCell>
                            )}
              ...
              {selectedSpell && (
                <Box>
                  ...
                  <Typography variant="subtitle1">
                    {(selectedSpell.stats.level > 0 ? "Nivel " + selectedSpell.stats.level : "Truco") +
                      ", " +
                    ...
                    <Box>
                      <Box component="p">
                        ...
                        {selectedSpell.stats.components.description && (
                          <Box component="span">{", " + selectedSpell.stats.components.description}</Box>
                        )}
                      </Box>
                    </Box>
                  )}
