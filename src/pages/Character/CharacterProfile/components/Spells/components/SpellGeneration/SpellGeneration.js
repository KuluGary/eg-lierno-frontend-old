...
export function SpellGeneration(props) {
    ...
    useEffect(() => {
        if (spellOptions.length === 0) {
            Api.fetchInternal("/spells").then((res) => setSpellOptions(res));
        }
    }, [spellOptions]);
    ...
    const addDescription = (description) => setSpellDescription(description);
    ...
    const saveNewSpell = () => {
        if (!addNew) {
            ...
            newSpell.stats.components = {
                ...newSpell.stats.components,
                type: componentsToAdd,
            };

            newSpell.stats.description = spellDescription;
    ...
    const validator = () => {
        return (
            !!spellToAdd?.name &&
            !!spellToAdd?.stats.school &&
            !!spellToAdd?.stats.castingTime &&
            !!spellToAdd?.stats.range &&
            !!spellToAdd?.stats.duration &&
            (!!spellDescription || !!spellToAdd?.stats.description)
        );
    };
    ...
    return (
        <Dialog maxWidth={"lg"} fullWidth open={props.open} className={classes.container}>
            ...
            <DialogContent>
                <Box style={{ width: "100%" }}>
                    <Box component="p" style={{ display: "flex", alignItems: "center" }}>
                        <Typography variant="caption">{"¿Crear un nuevo hechizo?"}</Typography>
                        <Checkbox color="default" checked={addNew} onClick={() => setAddNew(!addNew)} />
                    </Box>
                    {!addNew ? (
                        <>
                            <Box component="p">
                                <FormControl required style={{ width: "100%" }}>
                                    {spellOptions && (
                                        <Autocomplete
                                            ...
                                            renderInput={(params) => (
                                                <TextField {...params} label="Buscar hechizos..." variant="outlined" />
                                            )}
                                        />
                                    )}
                                </FormControl>
                                            ...
                                            <Checkbox
                                                ...
                                                onClick={() =>
                                                    components.includes("V")
                                                        ? setComponents(components.filter((item) => item !== "V"))
                                                        : setComponents([...components, "V"])
                                                }
                                                color="default"
                                            />
                                        }
                                            ...
                                            <Checkbox
                                                ...
                                                onClick={() =>
                                                    components.includes("S")
                                                        ? setComponents(components.filter((item) => item !== "S"))
                                                        : setComponents([...components, "S"])
                                                }
                                                color="default"
                                            />
                                        }
                                            ...
                                            <Checkbox
                                                ...
                                                onClick={() =>
                                                    components.includes("M")
                                                        ? setComponents(components.filter((item) => item !== "M"))
                                                        : setComponents([...components, "M"])
                                                }
                                                color="default"
                                            />
                                        }
            ...
            <DialogActions>
                ...
                <Button variant={"outlined"} disabled={!validator()} onClick={saveNewSpell} color="default">
                    Guardar
                </Button>
            </DialogActions>
        </Dialog>
